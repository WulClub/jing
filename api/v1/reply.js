...
var create = function (req, res, next) {
  ...
  ep.all('topic', 'topic_author', function (topic, topicAuthor) {
    Reply.newAndSave(content, topic_id, req.user._id, reply_id, ep.done(function (reply) {
      Topic.updateLastReply(topic_id, reply._id, ep.done(function () {
        ep.emit('reply_saved', reply);
        //发送at消息，并防止重复 at 作者
        var newContent = content.replace('@' + topicAuthor.loginname + ' ', '');
        at.sendMessageToMentionUsers(newContent, topic_id, req.user._id, reply._id);
      }));
    }));

    User.getUserById(req.user._id, ep.done(function (user) {
      user.score += 5;
      user.reply_count += 1;
      user.save();
      ep.emit('score_saved');
  ...
  ep.all('reply_saved', 'topic', function (reply, topic) {
    if (topic.author_id.toString() !== req.user._id.toString()) {
      message.sendReplyMessage(topic.author_id, req.user._id, topic._id, reply._id);
    }
