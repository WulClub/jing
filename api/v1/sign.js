...
exports.signup = function (req, res, next) {
    ...
    ep.on('prop_err', function (msg) {
        //res.status(422);
        res.send({status: 422, msg: msg, loginname: loginname, email: email});
    ...
    User.getUsersByQuery({
        ...
        tools.bhash(pass, ep.done(function (passhash) {
            ...
            var isActive=config.debug;

            User.newAndSave(loginname, loginname, passhash, email, avatarUrl, isActive, function (err) {
                if (err) {
                    return next(err);
                }
                if(!isActive)
...
exports.login = function (req, res, next) {
    ...
    ep.on('login_error', function (login_error) {
        //res.status(403);
        res.send({status: 403, msg: '用户名或密码错误'})
    ...
    getUser(loginname, function (err, user) {
        ...
        tools.bcompare(pass, passhash, ep.done(function (bool) {
            ...
            authMiddleWare.saveSession(user, req, res);
            ...
            res.send({status: 200, msg: '登录成功', data: {sessionid: req.sessionID}})
            //res.redirect(refer);
        }));
    });
};
...
exports.signout = function (req, res, next) {
    ...
    req.destroy(req.body.sessionid, function (a, ses) {

    });
    req.sessionStore.get(req.body.sessionid, function (a, ses) {
        console.log(343434);
...
exports.activeAccount = function (req, res, next) {
    ...
    User.getUserByLoginName(name, function (err, user) {
        ...
        if (!user || utility.md5(user.email + passhash + config.session_secret) !== key) {
            ...
            res.send({status: 500, msg: '信息有误，帐号无法被激活。'})
            return;//res.render('notify/notify', {error: '信息有误，帐号无法被激活。'});
        }
        if (user.active) {
            res.send({status: 404, msg: '帐号已经是激活状态。'});
        ...
        user.save(function (err) {
            ...
            res.send({status: 200, msg: '帐号已被激活，请登录'});
...
exports.updateSearchPass = function (req, res, next) {
    ...
    User.getUserByMail(email, function (err, user) {
        ...
        user.save(function (err) {
            ...
            res.send({status: 200, msg: '我们已给您填写的电子邮箱发送了一封邮件，请在24小时内点击里面的链接来重置密码。'});
...
exports.resetPass = function (req, res, next) {
    ...
    User.getUserByNameAndKey(name, key, function (err, user) {
        if (!user) {
            //res.status(403);
            res.send({status: 403, msg: '信息有误，密码无法重置。'});
    ...
    User.getUserByNameAndKey(name, key, ep.done(function (user) {
        ...
        tools.bhash(psw, ep.done(function (passhash) {
            ...
            user.save(function (err) {
                ...
                res.send({status: 200, msg: '你的密码已重置。'});
